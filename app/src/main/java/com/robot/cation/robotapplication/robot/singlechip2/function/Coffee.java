...
import static com.robot.cation.robotapplication.robot.singlechip2.TipsAndType.COFFEE_POSITION;
 ...
 * 咖啡
 */
public class Coffee extends BaseFun implements SingleChipeInfo {
    ...
    public void sendCommand(final PushBean.DataBean.OrderGoodsBean orderGoodsBean) {
        ...
        if (mcob) {
            ...
            voicePrompt = tipsText = "您的" + orderGoodsBean.getGoodsName() + "正在制作中请稍后!";
        ...
        mechanicalArm(TipsAndType.mainpulatorConfig.get(PICK_UP_CUP), new Interceptor.DataCallBack() {
            ...
            public void interceptor(byte[] command) {
                ...
                follCupMachine(new Interceptor.DataCallBack() {
                    ...
                    public void interceptor(byte[] command) {
                        //落完杯子
                        mechanicalArm(TipsAndType.mainpulatorConfig.get(COFFEE_POSITION), new Interceptor.DataCallBack() {
                            @Override
                            public void interceptor(byte[] command) {
                                //杯子已经抵达咖啡位置 开始制作咖啡
