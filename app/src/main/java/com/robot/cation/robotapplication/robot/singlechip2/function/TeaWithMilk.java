...
import static com.robot.cation.robotapplication.robot.singlechip2.TipsAndType.MILK_TEA_POSITION;
 ...
 * 奶茶
 */
public class TeaWithMilk extends BaseFun implements SingleChipeInfo {

    private static TeaWithMilk instance;
    ...
    public static TeaWithMilk getInstance() {
        if (instance == null) {
            instance = new TeaWithMilk();
        }
        return instance;
    }
    ...
    public void sendCommand(final PushBean.DataBean.OrderGoodsBean orderGoodsBean) {
        ...
        if (mcob) {
            voicePrompt = tipsText = "您的" + orderGoodsBean.getGoodsName() + "第" + mcobNumber + "正在出货中请稍后!";
            voicePromptFailed = tipsTextFailed = "您的" + orderGoodsBean.getGoodsName() + "第" + mcobNumber + "出货失败!";
        } else {
        ...
        mechanicalArm(TipsAndType.mainpulatorConfig.get(PICK_UP_CUP), new Interceptor.DataCallBack() {
            ...
            public void interceptor(byte[] command) {
                ...
                follCupMachine(new Interceptor.DataCallBack() {
                    ...
                    public void interceptor(byte[] command) {
                        //落完杯子
                        mechanicalArm(TipsAndType.mainpulatorConfig.get(MILK_TEA_POSITION), new Interceptor.DataCallBack() {
                            @Override
                            public void interceptor(byte[] command) {
                                //杯子已经抵达奶茶位置 开始制作奶茶
